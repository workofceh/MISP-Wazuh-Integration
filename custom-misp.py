...
#    alert = json.loads(line.strip())  # Read JSON line by line
with open(sys.argv[1], "r") as alert_file:
    for line in alert_file:
        ...
        try:
            ...
            print(f"JSON decoding error: {e}, Line: {line}")  # Debug output
            continue
...
misp_base_url = "https://<add your MISP server ip/domain>/attributes/restSearch/"
# MISP Server API AUTH KEY
misp_api_auth_key = "<read only auth key of MISP>"
# API - HTTP Headers
...
#print(f"DEBUG: alert JSON -> {json.dumps(alert, indent=2)}")  # Debugging
...
#if "rule" in alert and "groups" in alert["rule"]:
#    event_source = alert["rule"]["groups"][0] if len(alert["rule"]["groups"]) > 0 else "unknown"
#    event_type = alert["rule"]["groups"][2] if len(alert["rule"]["groups"]) > 2 else "unknown"
#else:
#    event_source = "unknown"
#    event_type = "unknown"
...
if event_source == "windows":
    ...
    try:
        ...
        if not wazuh_event_param:
            sys.exit()  # Exit if key is missing
